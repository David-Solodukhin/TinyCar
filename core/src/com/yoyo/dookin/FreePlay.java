...
public class FreePlay extends Game implements Screen, GestureListener {
	private World world;
	...
	public void render(float delta) {
		...
		if(car.fuel>0){
		shapeDebugger.rect(Gdx.graphics.getWidth()/2-(car.fuel * 3)/2, 50, car.fuel * 3, 10 * 4);}
		//shapeDebugger.rect(0, Gdx.graphics.getHeight()-200,3*50,3*50);
		shapeDebugger.end();

		hudBatch.begin();
		reverseIco.setScale(3);
		reverseIco.setPosition(70,Gdx.graphics.getHeight()-250);
		
		gasPedal.draw(hudBatch);
		
		coin.draw(hudBatch);
		fuel.draw(hudBatch);
		brakePedal.draw(hudBatch);
		reverseIco.draw(hudBatch);
	    pause.draw(hudBatch);
		font.setScale(1.9f);
		...
		if (tmp.length >= 500) {
			Vector2[] temp = new Vector2[tmp.length - 300];  //used to be tmp.length-300
			for (int i = 0; i < temp.length; i++) { //used to be i = 0;
				temp[i] = tmp[300 + i]; //used to be 300+i
			}
			tmp = temp;
		}
		if (Gdx.input.isTouched(2)) {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public void show() {
		...
		//car = new DroidCar(world, fixtureDef, wheelFixtureDef, 0, 3, 5, 1.25f,1, true); // 1
		car = new DroidCar(world, fixtureDef, wheelFixtureDef, 0, 3, 6, 1.25f,
				1, true);
		...
		brakePedal.setPosition(100,0);
		gasPedal.setPosition(Gdx.graphics.getWidth()-100-brakePedal.getWidth(),0);
