...
public class DemoLevel2 extends Game implements Screen, GestureListener {
	private World world;
	...
	public void render(float delta) {
		...
		if (!Gdx.input.isTouched(2)) {
			camera.position.set(car.getChassis().getPosition().x + 10, car.getChassis().getPosition().y + .004f * Gdx.graphics.getHeight(), 0);
			camera.update();
		}
		handleInput();
		hudBatch.begin();
	...
	//	hudBatch.draw(fgSprite3, fgParallax + (fgSprite2.getWidth() * 1.5f), fgParallay - 100, fgSprite2.getWidth() * 1.5f, fgSprite2.getHeight() * 1.5f);
		...
		if (car != null) {
			...
			if (camera.viewportHeight + tScale >= Gdx.graphics.getHeight() / 50 && camera.viewportHeight + tScale < 1900 / 50) {
				camera.viewportHeight += tScale;
				camera.viewportWidth = (camera.viewportHeight * (Gdx.graphics.getWidth() / 50f)) / (Gdx.graphics.getHeight() / 50f);

			}
			carBox.setPosition(car.getChassis().getPosition().x, car.getChassis().getPosition().y); // used
																									// to
																									// be
																									// setposition
		}
		...
		for (Body body : tmpBodies){
			...
			if( body.getType().equals(BodyType.DynamicBody) && body.getFixtureList().get(0).getShape().getType().equals(Type.Polygon) && !body.equals(car.getChassis()) && !body.equals(terrain))
			{
			//	PolygonShape tempShape = (PolygonShape) body.getFixtureList().get(0).getShape();
			//	Vector2 vertex = new Vector2();
			//	tempShape.getVertex(0, vertex);
			//	vertex = body.getWorldPoint(vertex);
				if(Math.abs((double)body.getPosition().x-(double)car.getChassis().getPosition().x)>100)
				{
					
				world.destroyBody(body); //optimize code don't know if it really helps?
				}
	...
	private void handleInput() {
		...
		if (Gdx.input.isTouched(1)) {
			...
			car.prevKeyUp();
		} else if (car.getChassis().getLinearVelocity().y > 1 && Gdx.input.isTouched(1)) {
			Gdx.app.log("hi", (Gdx.input.getPitch() + ""));
			car.prevD();

		} else if (car.getChassis().getLinearVelocity().y > 1 && Gdx.input.isTouched(0)) {
			Gdx.app.log("hi", (Gdx.input.getPitch() + ""));
			car.prevA();
	...
	private void update() {
		...
		if (tmpBodies.size > 1) // && drawnBodies.size() > 0
			for (int i = 0; i < tmpBodies.size; i++) {
				...
				/*
				 * if(isHeavy == true) { float sizeOf = 0; int counter = 0;
				 * for(int x = 0;x<tmpBodies.get(i).getFixtureList().size;x++) {
				 * sizeOf
				 * +=(tmpBodies.get(i).getFixtureList().get(x).getVertexCount
				 * ())*2; } float[] tempArr = new float[sizeOf]; for(int z =
				 * 0;z<tmpBodies.get(i).getFixtureList().size;z++) { for(int a =
				 * 0
				 * ;a<tmpBodies.get(i).getFixtureList().get(z).getVertexCount();
				 * a++) { Vector2 linePoints = new Vector2(); Vector2
				 * newLinePoints = tmpBodies.get(i).getWorldPoint(linePoints);
				 * tmpBodies.get(i).getFixtureList().get(z).getVertex(a,
				 * linePoints); tempArr[counter] = newLinePoints.x;
				 * tempArr[counter+1] = newLinePoints.y; counter+=2; } }
				 * if(tempArr.length>4) { shapeRendererF.polygon(tempArr); }
				 */
			}
		...
		if (justStarted) {
			...
			if (stuntFont.getScaleX() < 20) {
				stuntFont.setScale(stuntFont.getScaleX() + .2f, stuntFont.getScaleY() + .2f);
			...
			hudBatch.begin();
			stuntFont.draw(hudBatch, currentStunt, Gdx.graphics.getWidth() / 2 - 100, Gdx.graphics.getHeight() / 2);
			hudBatch.end();
		...
		if (!Gdx.input.isTouched()) {
			...
			if (drawPoints != null && drawPoints.length > 1 && !coinsGenerated) {
				...
				if (isSquare && drawPoints.length > 3) {
					...
					/*
					 * for (int x = 0; x < drawPoints.length; x+=6) { if (x
					 * !=1900) { // was 4 Vector2[] test = new Vector2[6]; //
					 * was 5 if ( x + 6 >= drawPoints.length &&
					 * drawPoints.length-x>=3) { ArrayList<Vector2> tester = new
					 * ArrayList<Vector2>(); while(x<drawPoints.length) {
					 * tester.add(drawPoints[x]); x++; } // test[5] =
					 * drawPoints[x + 5]; test = new Vector2[tester.size()];
					 * for(int zed = 0;zed<test.length;zed++) { test[zed] =
					 * tester.get(zed); } } else { test = new Vector2[6];
					 * test[0] = drawPoints[x]; test[1] = drawPoints[x + 1];
					 * test[2] = drawPoints[x + 2]; test[3] = drawPoints[x + 3];
					 * test[4] = drawPoints[x + 4]; test[5] = drawPoints[x + 5];
					 * 
					...
					for (int x = 0; x < terrainSplittr.size(); x++) {
						...
						// fixtureDef.shape = nelf2; //change back if this
						// fails..
						// fixtureDef.friction = .5f;
						// fixtureDef.restitution = 0;
						// testPolygonShape1.createFixture(fixtureDef);
						...
						// testPolygonShape1.createFixture(testPolygonShape2.getFixtureList().get(0).getShape(),
						// testPolygonShape2.getFixtureList().get(0).getDensity());
						// world.destroyBody(testPolygonShape2);

					}
					doop = testPolygonShape1;
					/*
					 * int i = 0; while (semiFinished.size() > 1 && i + 1 <
					 * semiFinished.size()) { Body testPolygonShape1 =
					 * semiFinished.get(i); if (i + 1 < semiFinished.size()) {
					 * Body testPolygonShape2 = semiFinished.get(i + 1); for
					 * (int x = 0; x < testPolygonShape2.getFixtureList().size;
					 * x++) { testPolygonShape1.createFixture(testPolygonShape2.
					 * getFixtureList().get(x).getShape(),
					 * testPolygonShape2.getFixtureList().get(x).getDensity());
					 * } world.destroyBody(semiFinished.get(i + 1));
					 * semiFinished.remove(i + 1); i++; } } i = 0; while
					 * (semiFinished.size() > 1 && i + 1 < semiFinished.size())
					 * { Body testPolygonShape1 = semiFinished.get(i); if (i + 1
					 * < semiFinished.size()) { Body testPolygonShape2 =
					 * semiFinished.get(i + 1); for (int x = 0; x <
					 * testPolygonShape2.getFixtureList().size; x++) {
					 * testPolygonShape1
					 * .createFixture(testPolygonShape2.getFixtureList
					 * ().get(x).getShape(),
					 * testPolygonShape2.getFixtureList().get(x).getDensity());
					 * } world.destroyBody(semiFinished.get(i + 1));
					 * semiFinished.remove(i + 1); i++; } } i = 0; while
		...
		if (car.fuel < 0) {
			...
			if (gassing) {
				car.fuel -= Math.abs(car.getChassis().getLinearVelocity().x) * .00791f + .1f;
			}
		}
	...
	public void resize(int width, int height) {
		...
		camera.viewportWidth = width / scaleFactor; // to make things
		// simpler i'm
		// making everything
	...
	public void show() {
		...
		groundShape.createChain(new Vector2[] { new Vector2(-5, 10), new Vector2(-5, 0), new Vector2(2, 0) });
	...
	public boolean tap(float x, float y, int count, int button) {
		...
		if (shapeType.getBoundingRectangle().contains(x, 1080 - y)) {
			...
			/*
			 * if (isSquare) { shapeType.setTexture(new Texture("light.png"));
			 * isSquare = false; isMisc = true; } else if (isMisc) { isMisc =
			 * false; isSquare = true; shapeType.setTexture(new
			 * Texture("heavy.png")); }
			 */

			if (isHeavy) {
				shapeType.setTexture(new Texture("light.png"));
				isHeavy = false;
			} else {
				shapeType.setTexture(new Texture("heavy.png"));
				isHeavy = true;
			}
	...
	public void setupMenu() {
		...
		if (car.fuel < 0) {
			...
			stuntFont.draw(hudBatch, "OUT OF INK!", Gdx.graphics.getWidth() / 2 - stuntFont.getBounds("SMOKED OUT!").width / 2, Gdx.graphics.getHeight() / 2);
	...
	public static Vector2[] removeDuplicates(Vector2[] arr) {
		 ...
		 * 0;i<arr.length;i++) { temporary.add(arr[i]); } for(int i =
		 * 0;i<temporary.size()-1;i++) {
		 * if(temporary.get(i).equals(temporary.get(i+1))) {
		 * temporary.remove(i+1); } } arr = new Vector2[temporary.size()];
		 * for(int i = 0;i<arr.length;i++) { arr[i] = temporary.get(i); } return
		 * arr;
		 */
	}
