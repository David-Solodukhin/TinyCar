...
public class GameModes extends Game implements Screen, GestureListener{
...
public Game mygame;
	...
	public void render(float delta)
	{
		...
		font.setScale(Gdx.graphics.getWidth()*.0065f);
		float titleWidth = font.getBounds("GAME MODES").width;
		
		font.setColor(Color.GREEN);
	     font.draw(menu, "GAME MODES", (Gdx.graphics.getWidth()-titleWidth)/2, Gdx.graphics.getHeight()-font.getBounds("T").height);
	     font.setColor(Color.WHITE);
	...
	public boolean tap(float x, float y, int count, int button) {
		...
		for(int i = 0;i<menuboxes.size();i++)
		{
			...
			{
				if(menuItems[i].contains("Free Play"))
				{
					currentItem = i;
					world.destroyBody(car.getChassis());
					car = null;
				...
				dispose();
				((Game) Gdx.app.getApplicationListener())
				.setScreen(new FreeDraw(mygame));
				break;
				}
				else if(menuItems[i].contains("BACK"))
				{
					currentItem = i;
				...
				dispose();
				((Game) Gdx.app.getApplicationListener())
				.setScreen(new DroidMenuScreen(mygame));
				break;
					
				
					
				}
				else if(menuItems[i].contains("FREE DRAW"))
				{
					currentItem = i;
					world.destroyBody(car.getChassis());
					car = null;
				...
				dispose();
				((Game) Gdx.app.getApplicationListener())
				.setScreen(new DemoLevel(mygame));
				break;
					
				}
				else if(menuItems[i].contains("FREE PLAY"))
				{
					currentItem = i;
					world.destroyBody(car.getChassis());
					car = null;
